...
void aggiungi_arco(grafo *g, int u, int v, long w)
{
    ...
    int u_parent = g->cCon[u];
    int v_parent = g->cCon[v];
    ...
    else
    {
        ...
        if (w < max_w)
        {
            ...
            arco old = {.u = max_u, .v = max_v, .weight = max_w};
...
int get_Lx(grafo *g, int start, bool *visited)
{
    int min_id = start;
...
arco *trova_best_arco_Lu_Lv(grafo *g, bool *Lu, bool *Lv)
{
    arco *best = NULL;
    for (int i = 0; i < g->hashSize; i++)
    {
        ...
        while (curr != NULL)
        {
            if (!curr->msf)
            {
                ...
                {
                    if(best == NULL || curr->weight<best->weight){
                        best=curr;
                    }
                }
            }
...
{ // true se concellato,false se arco non esiste
    ...
    if (!to_delete->msf)
    {
        ...
        vicini_cancella_arco(g, to_delete);
        ghash_cancella_arco(g, to_delete);
        g->n_archi--;
        return true;
    }
    // in msf
    g->costoMSF -= to_delete->weight;
    vicini_cancella_arco(g, to_delete);
    ghash_cancella_arco(g, to_delete);
    g->n_archi--;
    ...
    arco *best = trova_best_arco_Lu_Lv(g, Lu, Lv);
    if (best == NULL)
    {
...
int main(int argc, char **argv)
{
    ...
    if (n_thread <= 0 || hashSize <= 0 || n_max_mutex <= 0)
    {
    ...
    // fprintf(stderr, "n_archi:%d\tn_nodi:%d\t\n", n_archi, n_nodi);
    // stampa_archi_tmp(archi, n_archi);
    ...
    kruskal(g, archi);
    printf("%d %d %ld", g->n_archi, g->numCoCo, g->costoMSF);
    stampa_debug_vicini(g);
    cancella_arco(g,1,3);
    stampa_debug_vicini(g);
    // stampa_ghash(g);
    stampa_cCon(g);
